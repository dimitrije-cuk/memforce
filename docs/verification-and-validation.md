...
| Seed data | The first-launch state of MF-SDD-001, 3.5.5: **105 questions, 21 tags, 373 assignments and 93 alternative answers** — the 25 questions, 9 tags, 53 assignments and 27 alternative answers of the SQL fixture, plus the 80 questions, 12 tags, 320 assignments and 66 alternative answers of the carried question sets. This is the starting state of every manual procedure unless the procedure states otherwise. A change to the SQL fixture or to `app/src/main/assets/question-sets/` changes this baseline, and the procedures that quote a count must be re-anchored with it. |
...
MF-SRS-001 defines 126 requirement identifiers, one of which — REQ-USE-70 — is obsolete; the 125
in force each have exactly one verification entry. Distribution by
method:

| Method | Requirements | Where executed |
|---|---|---|
| Test (`T`) | 89 | JVM unit tests and the device suites of [clause 8](#8-test-suites-and-procedures) |
...
|---|---|---|
| **Unit** | Parsing, validation and merging (`com.memforce.importer`); the question sets carried in the package, the demo seed beside them and the pattern a typed criterion becomes (`com.memforce.db`); the search criteria and the statements built from them (`com.memforce.search`, `com.memforce.data`); answer marking and the rules of a game (`com.memforce.game`). Runs on the JVM, no device. | Automated: 10 test classes, 141 tests. |
...
|---|---|---|
| `com.memforce.importer.QuestionSetParserTest` | 45 | Format acceptance and the rejection rules it exercises: version support — both minor versions are read — missing required fields, unknown fields at both levels, non-object roots, blank and space-padded values, the question and tag length limits, byte order mark, trailing content, single-line tag names, duplicate tags within an array, an empty `questions` array, and the collection of all violations into one failure. For `alternativeAnswers`: the order of the file is kept, an empty array is accepted, and an array given without an answer or beside a blank one, a duplicate, a blank, an over-long or a wrongly typed entry, and a value that is not an array at all are each rejected — while a faulty `answer` is not blamed on the alternatives a second time. Parses the published template and example. Requirements: REQ-IMP-20, REQ-IMP-30, REQ-STD-30, REQ-POR-40. |
| `com.memforce.importer.MergedQuestionTest` | 10 | Set-level tags precede question-level tags; duplicate tags fold; questions without tags; file order preserved; a repeated question folds into one; the first answer wins; the alternative answers keep the order of the file, one repeating the answer is dropped, a repeated question unites the alternatives of every entry, and a question given none carries none. Requirements: REQ-IMP-70. |
...
| `com.memforce.game.AnswerMatcherTest` | 14 | What marking forgives — letter case, surrounding spacing, the length of spacing runs — and what it does not: a missing word, a different answer, an empty submission, and a question with no stored answer. These single-wording rules are checked through `matchesAny`, the one path the game marks by, so nothing is verified against a route the product does not take. For the wordings a question accepts: any of them is correct under the same comparison, one matching none of them is not, and neither an empty submission nor an empty set of wordings is ever correct. Requirements: REQ-GAME-80, REQ-QST-100. |
| `com.memforce.game.GameSessionTest` | 20 | The rules of a run: a game needs a question; the shuffle loses nothing and repeats for a fixed source; every other question is asked before one is asked again, whatever the outcome; the streak breaks on a wrong answer and on a skip; a question answered correctly twice counts once; an alternative answer is marked correct exactly as the answer is, and a submission matching neither is not; victory when every question has been answered correctly; perfect victory when the streak covers the whole set; no submission is taken once the run is decided. Requirements: REQ-GAME-60, REQ-GAME-70, REQ-GAME-80, REQ-GAME-90, REQ-GAME-100, REQ-GAME-110, REQ-GAME-120. |
| `com.memforce.db.BundledQuestionSetsTest` | 7 | Every question set carried in the installation package, held to MF-IFS-001 on the build machine: the folder is not empty, each file parses and declares the supported format version, each carries a name, a description and at least one set-level tag, none repeats a question text or folds to fewer questions than it lists, no alternative answer repeats the answer it varies, every question inherits the set-level tags, and the two shipped sets carry the counts and tags they are meant to. Requirements: REQ-IMP-120, REQ-IMP-130. |
| `com.memforce.db.DatabaseSeedTest` | 3 | The demo fixture `assets/seed/memforce_seed.sql`, read on the build machine as `DatabaseSeeder` reads it: it holds the 25 questions and 9 tags the seed-data baseline of [4.4](#44-resources-and-environment) counts, every assignment names a question and a tag the same file inserts, and every alternative answer names a seeded question, carries an identifier of its own and widens what that question accepts rather than repeating its answer or an earlier alternative — which `idx_alternative_answers_answer` would refuse on the first launch. Anticipates REQ-DB-110 on the build machine for the seeded rows; the fixture itself is not a question-set file, so REQ-IMP-130 does not reach it, and this is what keeps a bad seed row from reaching a device instead. |
...
Last executed on 2026-09-14 against the current draft: 141 tests, 0 failures, 0 errors, 0 skipped.
...
| **TS-QST** — question management | REQ-QST-10 … REQ-QST-110 | System | Manual |
...
| **TS-DB** — schema and integrity | REQ-DB-10 … REQ-DB-110 | Integration | Manual through the SQLite tool, procedure [8.3](#83-database-integrity) |
...
1. Confirm the five tables, their constraints and the two indexes exist as MF-SDD-001, 3.5.1 states.
...
4. Delete a question carrying two tags and two alternative answers; confirm exactly those two
   assignment rows and those two alternative-answer rows disappear and both tags remain
   (REQ-DB-70).
...
6. Insert, for one question, an alternative answer differing from one it already holds only by
   case; confirm the database — not the user interface — rejects it, and that the same text
   against a different question is accepted (REQ-DB-110).
7. Delete the highest-keyed question, insert a new one, and confirm the key is not reused
   (REQ-DB-30).
8. Close the application, restart the device, and confirm every record is still present
   (REQ-DB-90).
9. **Migration:** take a database file created by schema version 1 — a build from before
   `alternative_answers` existed — carrying questions, tags, assignments and accounts; install the
   current build over it; and confirm every one of those records is still readable, that
   `alternative_answers` and its unique index now exist and are empty, and that an alternative
   answer can be recorded against a question afterwards (REQ-DB-100, MF-SDD-001,
   [3.5.7](design.md#357-schema-evolution)). Confirm also that the demo rows and the carried sets
   are **not** re-seeded by the upgrade, and that a version for which no step is known raises
   rather than opening a database short of the schema.
...
classification and disposition live. An item stays in the table once it is closed, with its
disposition recorded, so that the register reads as a history rather than as a list that quietly
shortens.

| ID | Anomaly | Requirement | Severity | Disposition |
|---|---|---|---|---|
| **A-01** | `onUpgrade` dropped and recreated all tables, so raising the schema version would have destroyed every installation's data. | REQ-DB-100 | Major (latent — the code path never executed while the schema stood at version 1) | **Closed in version 1.1 of this plan.** `onUpgrade` now applies one versioned step per schema version in place (MF-SDD-001, [3.5.7](design.md#357-schema-evolution)), and a version with no step raises rather than opening a database short of the schema. Raising the version to 2 for `alternative_answers` is the first execution of that path. Regression cover: procedure [8.3](#83-database-integrity) step 9, which is now executable and must pass at every schema change; it replaces the blocking condition this anomaly carried. |
...
| **A-06** | No automated test executes a DAO, the schema, `PasswordHasher`, `Session`, `Lobby` or any screen. The statements the DAOs run are covered on the JVM, but nothing automated runs them against SQLite. | Verification coverage | Major (process) | **Open — planned.** Add database integration tests first ([7.3](#73-automated-test-inventory)); until then the manual suites are mandatory per change, as [4.3](#43-master-schedule) requires. The migration of A-01 and the uniqueness of REQ-DB-110 fall in this gap and are verified by [8.3](#83-database-integrity) steps 6 and 9 by hand. |
...
|---|---|---|---|
| Are the requirements verified? | Requirement verification coverage: requirements with a passing record ÷ 125 | Per release candidate | Must be 100 % before a release baseline. |
