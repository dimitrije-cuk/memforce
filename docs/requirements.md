...
| REQ-SRCH-170 | The application shall allow the user to choose, from the home screen's menu, whether a question in a result list displays the tags it carries; it shall display them until the user chooses otherwise, shall apply the choice wherever the search of REQ-SRCH-150 is presented, shall retain it until the user changes it, and shall leave the tags offered under REQ-SRCH-110 displayed in either case. | F | M | SN-03, D-31 |
...
implementations of the same idea drift apart, and a user would have to learn each. REQ-SRCH-170
is what lets a user who reads by question text alone put the tags away: it governs the tags *in*
a result, which are description, and deliberately not the tags offered beneath the field, which
are the controls that narrow the search — hiding those would take away the only means of removing
a criterion already held. The choice is retained rather than asked again each time, because a user
who has decided how the list should read has not decided it only for this sitting, and it holds on
both presentations of REQ-SRCH-150, since a search that showed different rows on two screens would
be the drift that requirement exists to prevent.
...
| REQ-SRCH-170 | T | On a first launch the results show each question's tags. Turning **Show question tags** off leaves the same questions in the same order, with the same selection, and no tags within them; the tags offered beneath the search field are unchanged. The question list screen, opened afterwards, also shows no tags within its questions. The choice survives leaving and re-entering the application, and signing out and in again; turning it back on restores the tags. |
...
| SN-03 | A learner finds a question again by what it says or by what it is about, in a library that keeps growing. | REQ-SRCH-10, REQ-SRCH-20, REQ-SRCH-30, REQ-SRCH-50, REQ-SRCH-60, REQ-SRCH-90, REQ-SRCH-100, REQ-SRCH-110, REQ-SRCH-120, REQ-SRCH-150, REQ-SRCH-160, REQ-SRCH-170, REQ-EXT-40, REQ-QST-50, REQ-QST-80, REQ-TAG-50, REQ-TAG-70, REQ-TAG-90 |
