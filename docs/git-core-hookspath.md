...
| Field | Value |
|---|---|
| Document title | MemForce — Git `core.hooksPath` |
| Document identifier | MF-PRC-002 |
| Version | 1.0 |
| Date | 2026-09-10 |
| Status | Background note supporting [MF-PRC-001](commit-message-workflow.md) |
...
Pointing `core.hooksPath` at `.githooks` therefore disables any hooks already installed under
`.git/hooks` in a clone. Check that directory for non-sample files before running
`setup-hooks.bat`, and move anything worth keeping into `.githooks`.
...
husky and similar tools configure the same key, and the last one to run wins. If a clone already uses one of them, pointing Git at `.githooks` disables their hooks, and reinstalling them later points Git away from `.githooks`.
